...
def upsert_flights(flights: list):
    ...
    for f in flights:
        c.execute("""
            ...
            (icao, callsign, lat, lon, altitude, speed, heading, origin, destination, aircraft_type, fetched_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (f.get("icao"), f.get("callsign"), f.get("lat"), f.get("lon"),
              f.get("altitude"), f.get("speed"), f.get("heading"),
              f.get("origin"), f.get("destination"), f.get("aircraft_type"), now))
    conn.commit()
    conn.close()
...
def upsert_ships(ships: list):
    ...
    for s in ships:
        c.execute("""
            ...
            (mmsi, name, lat, lon, speed, heading, ship_type, flag, fetched_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (s.get("mmsi"), s.get("name"), s.get("lat"), s.get("lon"),
              s.get("speed"), s.get("heading"), s.get("ship_type"), s.get("flag"), now))
    conn.commit()
    conn.close()
